  ...
  useEffect(() => {
    const fetchFarmacos = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8000/farmacos");
        setFarmacos(response);  // Accede a la propiedad 'farmacos'
      } catch (err) {
        setError("Error al obtener los fármacos: " + err.message);
      }
    };
      ...
      <table>
        ...
        <tbody>
          {farmacos.map(farmaco => (
            <tr key={farmaco.id}>
              <td>{farmaco.nombre_comercial}</td>
              <td>{farmaco.fecha_elaboracion}</td>
              <td>{farmaco.principio_activo}</td>
              <td>{farmaco.miligramos}</td>
            </tr>
          ))}
        </tbody> */}
      {/* </table> */}
